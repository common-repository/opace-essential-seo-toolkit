...
Tags: opace, seo tool, seo audit, seo plugin, seo analysis, social signals, backlink analysis, seo auditing, speed analysis, pingdom, gt metrix, google pagespeed, alexa, semrush, majestic, ahrefs, moz,  copyscape, siteliner, responsinator, structured data testing tool, mobile friendly test, thinkwith google test, xml sitemap generator
...
Once the Plugin is installed and activated you will need to go set up the plugins page, search for the plugin and click settings.
...
* See [the Chrome extension](https://chrome.google.com/webstore/detail/essential-seo-toolkit-seo/icagkiolfkmndbggheneeamfbnobcdma?hl=en)
...
= 1.2.6 =
- Removed tools that are no longer supported - Alexa, Think With Google, Structured Data Testing Tool
- Added the following tools - Rich Testing Tool, Ubersuggest, SpyFu, Google Trends, BrowseSEO, Social Searcher and Open Graph Meta Tag Checker
- Replaced Social Share Counter tool 
- Improved input validation and sanitisation in the `generate_link` and `generate_link_parts` functions.
- Added output sanitisation using `esc_url`, `esc_html__`, and `esc_attr__` in the `display_metabox` function.
- Implemented search/filter functionality for the SEO categories and tools using a search box. Users can now search for specific tools or categories by typing a keyword in the search input field. The list of tools and categories will be filtered based on the entered keyword, improving the user experience and making it easier to find the desired tool or category. The category boxes will also collapse back when the search phrase is deleted.
- Functions added to improve maintenance when adding and testing new default sources and categories.
