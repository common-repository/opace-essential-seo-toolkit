...
* Description:      The Opace Essential SEO Toolkit is a collection of useful SEO tools, which can be accessed using any post type. Simply click and open your URL using a wide range of SEO tools.
* Version:          1.2.6
    ...
    public function admin_scripts($hook) {
      ...
      */
      if ( $this->is_screen_supported() ) {
        ...
        */
        //Register
        wp_register_script( 'eseot-meta', $this->plugin_url.'assets/js/eseot-meta.js', array('jquery'), $this->version, true);
		wp_register_script( 'eseot-meta-custom', $this->plugin_url.'assets/js/eseot-meta-custom.js', array('jquery'), $this->version, true);
        //Enqueue
        wp_enqueue_script( 'eseot-meta' );
		wp_enqueue_script( 'eseot-meta-custom' );
    ...
    */
	public function display_metabox($post){
	    ...
	    printf('<p>%3$s <a href="%1$s" title="%2$s" target="_blank">%2$s</a> %4$s:</p>',
	        ...
	        esc_html__('Open', $this->plugin_textdomain),
	        esc_html__('with', $this->plugin_textdomain)
	    );

	    // Add the search/filter input field
	    ?>
	    <input type="text" id="eseot-search" placeholder="<?php esc_attr_e('Search SEO tools...', $this->plugin_textdomain); ?>" />
	    <?php
