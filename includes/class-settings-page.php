...
if(!class_exists( 'opace_essential_seo_toolkit_settings' )) {
  class opace_essential_seo_toolkit_settings {
    ...
    function __construct() {
      ...
      // CODE FOR EDITING NEW SOURCES/CATEGORIES AND TESTING ONLY
      //add_action('admin_init', array($this, 'manual_update'));
    ...
    */
    ...
    public function update_categories() {
      ...
      // Update the option with the updated categories array
      update_option('eseot_categories', $updated_categories);
    }
    
    public function update_sources() {
      // Define the updated sources array
      ...
      // Update the option with the updated sources array
      update_option('eseot_sources', $updated_sources);
    }
    ...
    */
    public function get_default_options( $single_setting = false ) {
      $default_settings = array(
        ...
        'eseot_categories' => get_option('eseot_categories'),
        'eseot_sources' => get_option('eseot_sources'),
      );
    ...
    */
    public function update_options() {
        ...
        // New version update (e.g., 1.2.6)
        if ($this->version !== get_option('eseot_version') && $this->version == '1.2.6') {
          // Call the update_categories() and update_sources() functions to update the options
          $this->update_categories();
          $this->update_sources();

          // Update the version number in the options
          ...
          if( ! empty ( $args['options'] ) && is_array( $args['options'] ) ){
            ...
            foreach( $args['options'] as $key => $label ){
              ...
              $markup .= sprintf( '<label for="enable-%1$s"><input id="enable-%1$s" type="checkbox" name="%2$s[%1$s]" value="1" %3$s> %4$s</label><br/>', 
                ...
                checked( isset($value[$key]) ? $value[$key] : 0, 1, false ),
